...
#   
  ...
  #---------------------------Shillong Times-----------------------------------
   ...
   for (i in 1:length(links4)){
    a<-html(links4[i])
    heading[i]<-a %>% html_node("#content h1") %>% html_text()    #Read heading
    #print(heading[i])                               
    content[i]<-a %>% html_nodes(".entry-content p") %>% html_text()      #Takes each paragraph as an item in a list
    #content[i]<-paste(content_list,sep=" ",collapse = "")
    #print(content[i])
    date[i]<-a %>% html_node(".entry-date") %>% html_text()
    #print(date[i])
    print(i)
  }
  df<-data.frame(heading,date,content)
  names(df)<-c("Heading","Date","Content")
  write.csv(df,"ShillongTimes.csv")
  print("Exported to csv format")
  #------------------------------------------------------------------------
